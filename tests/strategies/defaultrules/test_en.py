...
def test_apply_en() -> None:
    ...
    assert rules_strategy.get_lemma("preserves", "en") == "preserve"
    # dropped (below the 99% bar): ries/ties (-erie/-tie), esses (finesse), trices
    assert rules_strategy.get_lemma("nurseries", "en") is None
    assert rules_strategy.get_lemma("realities", "en") is None
    assert rules_strategy.get_lemma("mistresses", "en") is None
    assert rules_strategy.get_lemma("matrices", "en") is None
