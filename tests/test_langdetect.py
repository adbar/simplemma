...
def test_detection():
    ...
    # sanity checks
    assert lang_detector.lang_detector(" aa ", lang=("de", "en"), extensive=True) == [
        ("unk", 1)
    ]
    text = "Test test"
    assert lang_detector.lang_detector(text, lang=("de", "en"), extensive=False) == [
        ("unk", 1)
    ]
    assert lang_detector.lang_detector(text, lang=("de", "en"), extensive=True) == [
        ("unk", 1)
    ]
    # language detection
